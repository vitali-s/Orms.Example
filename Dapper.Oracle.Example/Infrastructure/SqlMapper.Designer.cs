...
namespace Dapper
{
    ...
    static partial class SqlMapper
    {
        ...
        static SqlMapper()
        {
            ...
            typeMap[typeof(Guid)] = DbType.Binary;
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
        ...
        internal static Func<IDataReader, object> GetDapperRowDeserializer(IDataRecord reader, int startBound, int length, bool returnNullIfFirstMissing)
        {
            ...
            if (fieldCount <= startBound && length != 0)
            {
                ...
                if (item != null)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (unboxType == typeof(Guid) && dataType == typeof(byte[]))
                            {
                                unboxType = typeof(byte[]);

                                il.Emit(OpCodes.Unbox_Any, unboxType); // stack is now [target][target][typed-value]

                                if ((item.MemberType == typeof(Guid) && unboxType == typeof(byte[])) || (nullUnderlyingType != null && nullUnderlyingType.IsEnum))
                                {
                                    il.Emit(OpCodes.Newobj, item.MemberType.GetConstructor(new[] { nullUnderlyingType ?? unboxType }));
                                }
                            }
                            else if (dataType == unboxType || dataTypeCode == unboxTypeCode || dataTypeCode == Type.GetTypeCode(nullUnderlyingType))
                            {
                                il.Emit(OpCodes.Unbox_Any, unboxType); // stack is now [target][target][typed-value]
                            }
                            else
